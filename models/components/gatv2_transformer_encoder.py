...
class gatv2_Exp_InterpretableTransformerEncoder(TransformerEncoderLayer):
    def __init__(self, d_model, nhead, config, dim_feedforward=2048, dropout=0.1, activation=F.relu,
        ...
        self.self_attn = gatv2_Exp_MultiheadAttention(d_model, nhead, config, dropout=dropout, batch_first=batch_first,
                                            **factory_kwargs)
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        x = src
        if self.norm_first:
            # x = x + self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, attn_map_bias = attn_map_bias, attn_solid_mask=attn_solid_mask)
            # x = x + self._ff_block(self.norm2(x))
            
            x = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, attn_map_bias = attn_map_bias, attn_solid_mask=attn_solid_mask)
            ...
            # x = x * self._sa_block(x, src_mask, src_key_padding_mask, attn_map_bias = attn_map_bias, attn_solid_mask=attn_solid_mask)

        return x
    


class gatv2_Exp_MultiheadAttention(nn.MultiheadAttention):
    def __init__(self, embed_dim, num_heads, config, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        # dim_gatv2_a = int(2*embed_dim/num_heads)
        # print(f'dim_gatv2_a={dim_gatv2_a}')
        # self.gatv2_a = nn.Parameter(torch.empty(num_heads, dim_gatv2_a))
        self.gatv2_a = nn.Parameter(torch.empty(num_heads, 32))

        init.xavier_uniform_(self.gatv2_a)

        self.linear_gatv2 = nn.Linear(2*embed_dim, num_heads*32)
        # self.linear_gatv2 = nn.Linear(2*embed_dim, 2* embed_dim)
    ...
    def Exp_multi_head_attention_forward(
        ...
        attn_output, attn_output_weights, orig_attn = self._scaled_dot_product_attention(query, key, v, attn_mask, dropout_p, attn_map_bias, attn_solid_mask)
    ...
    def _scaled_dot_product_attention(
        ...
        # here, Q, K, V are the original input feature x 

        # print(f'q.shape={q.shape}, k.shape={k.shape}, v.shape ={v.shape}') # (360, 16, 360)
        
        q = q.transpose(0,1) # (16, 360, 360)
        # print(f'after transpose q.shape={q.shape}')
        
        # concate
        concat_matrix = self.concat_features(q, q)  # (16, 360, 360, 360 + 360)
        # print(f'concat_matrix.shape={concat_matrix.shape}')

        # linear transformation
        X = self.linear_gatv2(concat_matrix)   # (16, 360, 360, 2 * 32)
        X = F.leaky_relu(X, 0.2)
        
        # print(f'X.shape={X.shape}')

        bz, num_nodes, _ = q.shape

        X_reshaped = X.view(bz, num_nodes, num_nodes, self.num_heads, -1)  # (16, 360, 360, 2, 32)
        # print(f'X_reshaped.shape={X_reshaped.shape}')

        X_reshaped = X_reshaped.permute(0, 3, 1, 2, 4).reshape(bz*self.num_heads, num_nodes, num_nodes, -1) 
        # print(f'After X_reshaped.shape={X_reshaped.shape}')   # (([32, 360, 360, 32])
        
        expanded_gatv2_a = self.gatv2_a.repeat(bz, 1)  # (2, 32) -> (32, 32)
        # print(f'expanded_gatv2_a.shape={expanded_gatv2_a.shape}')

        expanded_gatv2_a = expanded_gatv2_a.unsqueeze(1).unsqueeze(2)  # (32, 1, 1, 32)
        # print(f'expanded_gatv2_a_unsqueeze.shape={expanded_gatv2_a.shape}')

        attn = (X_reshaped * expanded_gatv2_a).sum(dim=-1)  # (32, 360, 360)
        ...
        # output = torch.ones(int(bz*self.num_heads), num_nodes, int(num_nodes/self.num_heads)).cuda()


        return output, attn, attn
    

    def concat_features(self, X, Y):
        ...
        concat_result = torch.cat((X_expanded.expand(-1, -1, num_nodes, -1), 
                    Y_expanded.expand(-1, num_nodes, -1, -1)), dim=-1)
        
        assert concat_result.shape == (batch_size, num_nodes, num_nodes, 2 * feature_dim), 'concat feature dim is incorrect'
